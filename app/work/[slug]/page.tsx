...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main>
      ...
      <section className="section shell case-layout"><aside className="case-nav"><span>CASE STUDY</span><a href="#problem">Problem</a><a href="#solution">Solution</a><a href="#features">Features</a>{project.pluginNote && <a href="#plugin">Plugin</a>}<a href="#outcome">Outcome</a></aside><div className="case-content">
        ...
        {project.pluginNote && <section id="plugin" className="case-block"><p className="eyebrow">04 · MOODLE PLUGIN</p><h2>Companion plugin: local_supertools</h2><p>{project.pluginNote}</p><div className="tags large"><span>local_supertools</span><span>Moodle Local Plugin</span><span>Custom Web Services</span><span>Capabilities</span></div></section>}
        <section className="case-block"><p className="eyebrow">{project.pluginNote ? "05" : "04"} · TECHNOLOGY</p><h2>Tools and platform</h2><div className="tags large">{project.stack.map(t=><span key={t}>{t}</span>)}</div></section>
        <section id="outcome" className="outcome"><p className="eyebrow pale">{project.pluginNote ? "06" : "05"} · OUTCOME</p><h2>{project.outcome}</h2></section>
