...
import ProjectGalleryMedia from "../../project-gallery-media";
...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
  ...
  const isTrainingPortfolio = slug === "moodle-teacher-training";
  ...
  return <main>
    ...
    {project.liveUrl ? <section className="shell project-live-detail"><div className="live-detail-frame"><iframe src={project.liveUrl} title={`${project.title} live website`} loading="lazy" sandbox="allow-scripts allow-same-origin allow-forms" /><div className="live-preview-topline"><span className="live-dot" /> LIVE WEBSITE</div><a className="media-click-layer" href={project.liveUrl} target="_blank" rel="noreferrer" aria-label={`Open ${project.title} live website`} /></div><p className="muted live-detail-note">This is a live preview of the deployed project, not a recreated portfolio mockup.</p></section> : images.length>0 ? <section className={`shell project-gallery${isTrainingPortfolio ? " training-gallery" : ""}`}>{images.slice(0,16).map((src,i)=><ProjectGalleryMedia key={`${src}-${i}`} src={src} projectTitle={project.title} projectSlug={slug} index={i} />)}</section> : isTrainingPortfolio ? <section className="shell internal-media-note training-media-note"><p className="eyebrow">TRAINING DOCUMENTATION</p><h2>Gallery ready for up to 16 activity photos.</h2><p className="muted">This section is prepared for real documentation from Moodle workshops and other teacher technology training sessions. Photos can be added later without changing the portfolio structure.</p></section> : <section className="shell internal-media-note"><p className="eyebrow">PROJECT MEDIA</p><h2>No fabricated screenshot.</h2><p className="muted">This project contains internal or private operational data. The case study documents the real workflow and implementation without displaying invented interface imagery.</p></section>}
